...
async function saveData() {
  const data = { projects, weeklyTasks, oneOffTasks, recurringTasks, deletedTasks,
                 shoppingList, longTermList, canBuyList, nextId };
  try {
...
function renderWeekly() {
  ...
  const openTasks = weeklyTasks.filter(t => t.status === 'open' &&
    (filterProjects.length === 0 || filterProjects.includes(t.project)));
...
function taskCell(task) {
  ...
  cell.innerHTML = `<span class="task-name">${task.name}</span><br>` +
    `<span class="task-meta" style="color:${color}">${task.project}</span>`;
